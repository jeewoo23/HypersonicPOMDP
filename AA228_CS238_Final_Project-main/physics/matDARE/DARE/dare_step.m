...
% DARE_STEP  Run one full DARE simulation and return key state quantities.
%
%   Inputs:
%     equivalence_ratio  - combustor equivalence ratio (fuel control input)
%     mach_flight        - freestream Mach number
...
%     mach_mixture       - mixture Mach number at solver exit (Mach indicator)
%     x_norm             - normalised solver progress: x_exit / (comb_len + nozzle_len)
%                          ~1.0 means run completed, <1 means early termination
%     success            - 1 if run reached full length, 0 otherwise
...
addpath(fullfile(base, 'Examples', 'Ramjet', '1'));  % non-interactive nozzleModule
...
% Engine geometry (fixed design from Ramjet Example 1)
