...
nozzOut = cell(8,1);   % or larger if later indices are used
...
%% Record output
nozzOut{1} = choice;   % which path we took
nozzOut{2} = x;        % x-grid / independent var (if used)
nozzOut{3} = y;        % state history / final state

if exist('P_Desired_Exit','var') && exist('T_Nozzle_Exit','var') ...
   && exist('Velocity_Nozzle_Exit','var') && exist('Area_Nozzle_Exit','var')
    % Perfect-expansion / target-exit-pressure branch
    nozzOut{4} = P_Desired_Exit;
    nozzOut{5} = T_Nozzle_Exit;
    nozzOut{6} = Velocity_Nozzle_Exit;
    nozzOut{7} = Area_Nozzle_Exit;
else
    % Numerical-solve branch: pull at-nozzle-exit values from y
    % (these indices match the example implementations)
    nozzOut{4} = y(3);   % Pressure at nozzle exit
    nozzOut{5} = y(4);   % Temperature at nozzle exit
    nozzOut{6} = y(2);   % Velocity at nozzle exit
    nozzOut{7} = y(19);  % Area at nozzle exit (or effective A)
end
